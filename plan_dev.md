...
| 6 | **Code Graph** — graphe de connaissances projet (inspiré graphify) | ✅ Implémenté V1+V2+V2.1 (2026-08) | [`spec_code_graph.md`](spec_code_graph.md) · graphe structurel local sans LLM (nœuds/arêtes `calls`/`imports`/`inherits`, EXTRACTED/INFERRED) → injecté à pi/plh (mode A sous-graphe scoré + mode B wiki `.pilot/graph-wiki/`). Paramétrable (⚙️ → Graphe, backend `heuristic`/`treesitter`), bouton 📊 rebuild, mise à jour incrémentale auto (lazy à la query + watcher V2.1). V1 heuristique ✅ + V2 tree-sitter ✅ + V2.1 watcher ✅ |
