...
|---|---|---|
| Détection d'anomalies (tâche 8) | [`spec_anomaly.md`](spec_anomaly.md) | ✅ Implémenté (2026-08) — surveillance arrière-plan sans LLM : observateur RPC combiné (activité par projet + anomalie par agent), moniteur thread qui signale un agent actif sans progression depuis le seuil (défaut 30 min), événement `agent-anomaly` → bandeau + notification desktop, agent de diagnostic dédié (`diagnostic`) qui PROPOSE des évolutions (validation utilisateur, aucune action auto). Réglages (activation + seuil) dans Paramètres. Voir `spec_anomaly.md` |
...
| 2 | **#16** — Notifications desktop pour l'Assistant 🧭 (tâche déléguée terminée / anomalie) | ✅ Implémenté | `spec_super_agent.md` · réglage `notify_super_agent_done` (Paramètres → Assistant, défaut off), `notifySuperAgentDone()` (desktop-notify.js) branché sur la délégation terminée + connexion perdue |
| 2 | **Évolution 3** — Paramètre assistant « réponses courtes » | ✅ Implémenté | `spec_super_agent.md` · réglage `super_agent_concise` (Paramètres → Assistant, défaut off) injecté dans le prompt système (canal streaming + bloquant) |
| 2 | **#13** — Outil `schedule` assistant (relances différées/périodiques) | ✅ Implémenté | `spec_super_agent.md` · extension `pilot-assistant-schedule.ts` (schedule_create/list/delete), table `assistant_schedules`, ticker frontend 10 s (1 fire/tick, pas de tick si session morte), garde-fous every ≥ 60 s / max 20 |
| 2 | **Évolution 2** — Mode Assistant sur le web-remote (minimaliste) | ✅ Implémenté (MVP) | `spec_web_remote.md` §5.1 · sélecteur de mode 🧭/🤖 (défaut assistant, persisté localStorage), route `POST /api/superagent/prompt`, événements super-agent taggés `__channel:superagent` sur le WS web |
...
| 6 | **Code Graph** — graphe de connaissances projet (inspiré graphify) | ✅ Implémenté V1+V2+V2.1+Option C (2026-08) | [`spec_code_graph.md`](spec_code_graph.md) · graphe structurel local sans LLM (nœuds/arêtes `calls`/`imports`/`inherits`, EXTRACTED/INFERRED) → injecté à pi/plh (mode A sous-graphe scoré + mode B wiki `.pilot/graph-wiki/`). Paramétrable (⚙️ → Graphe, backend `heuristic`/`treesitter`), mise à jour incrémentale auto (lazy à la query + watcher V2.1). **Option C ✅** : onglet « Graphe » dédié (remplace la modale 📊) + visualisation 2D interactive `force-graph` (pan/zoom, clic nœud → ouvre fichier, survol, coloration, filtres, sous-graphe contextuel) + commande Rust `graph_export`. V1 ✅ + V2 ✅ + V2.1 ✅ + Option C ✅ |
