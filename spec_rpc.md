...
| `agent_settled` | **Bus d'agents (orchestration, `agents-bus.js`) uniquement** : settle définitif du tour pi. Traité comme filet **« zéro perte »** : finalise un tour dont l'`agent_end` final serait perdu (idempotent, garde anti cross-talk via le marqueur `startedByAgent` posé à l'`agent_start` de la run). Événement sans payload — l'état vient de l'historique de la run. |
