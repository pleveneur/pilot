...
impl InnerWebView {
  ...
  fn init_webview(
    ...
    // Permission handler (patched for Pilot).
    // Upstream wry 0.55.1 only attaches this handler when `attributes.clipboard` is
    // true, and only ever grants CLIPBOARD_READ — so the Web Speech API / getUserMedia
    // is blocked with `not-allowed` on the desktop WebView. Pilot always attaches the
    // handler and grants MICROPHONE (the dictée-vocale feature) unconditionally, while
    // preserving the original clipboard behavior behind its flag.
    let clipboard_enabled = attributes.clipboard;
    unsafe {
      webview.add_PermissionRequested(
        &PermissionRequestedEventHandler::create(Box::new(move |_, args| {
          ...
          } else if kind == COREWEBVIEW2_PERMISSION_KIND_CLIPBOARD_READ && clipboard_enabled {
            args.SetState(COREWEBVIEW2_PERMISSION_STATE_ALLOW)?;
          }
