...
- Publication : workflow GitHub Actions `.github/workflows/release.yml` (tag `v*`) → build multi-plateforme (Windows NSIS, macOS DMG x86_64/aarch64, Linux AppImage) + génération du changelog depuis les commits (`scripts/update-release-notes.js`, mis à jour sur le body de la release) + génération de `latest.json` (`scripts/gen-latest-json.js` qui injecte ce body dans `notes`) uploadé sur la release.
