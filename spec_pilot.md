...
- **Avatar (PLface) au lancement** : réglage « Lancer mon avatar (PLface) au démarrage » (`plface_autostart_enabled`, défaut **false** ; chemin `plface_exe_path`, défaut vide) dans Paramètres → Général → Démarrage. Au démarrage de Pilot, si l'option est active et le chemin renseigné, Pilot sonde l'API locale de PLface (`http://127.0.0.1:3000/status`, timeout court < 1 s) ; si elle ne répond pas, il lance l'exécutable **détaché** (PLface survit à la fermeture de Pilot). Jamais bloquant, fail-open : un utilisateur sans PLface ne voit aucune différence. Champ d'exploration « Parcourir… » et bouton « Tester maintenant » (commande `check_and_launch_plface`) qui affiche un message **clair et non technique** pour chaque état : déjà lancé, lancé, exécutable introuvable, désactivé, échec du lancement. Moteur : [`src-tauri/src/plface.rs`](src-tauri/src/plface.rs) (logique de décision pure `decide_launch`).
...
- **POC minimal** (transport stdio uniquement — pas de http/sse) : Pilot peut se connecter à un **serveur MCP externe** (stdio) via une **extension pi dédiée** qui embarque le **SDK MCP bundlé** (esbuild).
...
- `mcp_config.rs` lit/écrit `mcp.json` dans `app_data_dir` (**pas AppConfig**) : `McpServer { id, name, command, args, env, enabled }`. L'extension choisit le serveur : `PILOT_MCP_SERVER` (serveur cible désigné) s'il est `enabled` `stdio`, sinon fail-back le **1er serveur stdio activé** (rétrocompat). Découvre `tools/list` et enregistre chaque outil en `mcp_<serverId>_<name>` (fail-open).
...
- Commandes : `mcp_list_servers`, `mcp_save_servers`, `mcp_test_connection` (handshake `initialize`), `mcp_set_enabled` (persiste `mcp_enabled` dans AppConfig), `mcp_set_agent_confirm`, `mcp_get_state`.
- **UI Paramètres « Serveurs MCP »** : onglet dédié (case globale activer/désactiver MCP → `mcp_set_enabled` ; case « Demand une confirmation utilisateur avant qu'un agent utilise un serveur MCP » → `mcp_set_agent_confirm` ; lister/ajouter/modifier/supprimer les serveurs → `mcp_list_servers`/`mcp_save_servers` ; bouton « Tester » par serveur → `mcp_test_connection` ; suppression confirmée). Le transport est fixé à `stdio` (pas de http/sse). Helpers purs testés dans `src/js/mcp-utils.js` (+ `.test.js`).
- Dette technique à lever (post-POC) : transports non-stdio, résolution des collisions de noms d'outils.
